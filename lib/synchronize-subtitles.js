...
export default async function (filePath, syncFolder, target, options = {}) {
  ...
  const validSyncedSubtitles = getSynchronizationDelays(haystackFingerprints, syncedSubtitles, fingerprinter, options)
  if (validSyncedSubtitles.length === 0) return

  for (const syncData of validSyncedSubtitles) {
    log(
      syncData.subtitlePath,
...
export function getSynchronizationDelays (haystackFingerprints, syncedSubtitles, fingerprinter, options) {
  ...
  for (const syncData of syncedSubtitles) {
    ...
    const minimalMatchCount = options.minimalMatchCount ?? 10
    const maxAllowedDeviation = options.maxAllowedDeviation ?? 750
    const maxTargettedDeviation = options.maxTargettedDeviation ?? 200
    const filterFactorMin = options.filterFactorMin ?? 25
    const filterFactorMax = options.filterFactorMax ?? 40
    const filterFactorDelta = options.filterFactorDelta ?? 5
    let filteredDelays
    let stdDeviation

    for (let filterFactor = filterFactorMin; filterFactor < filterFactorMax; filterFactor += filterFactorDelta) {
      const currentFiltered = filterOutliers(delays, filterFactor / 100)
