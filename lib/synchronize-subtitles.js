...
export function getSynchronizationDelays (haystackFingerprints, syncedSubtitles, fingerprinter, options) {
  ...
  for (const syncData of syncedSubtitles) {
    ...
    const filterFactorMin = options.filterFactorMin ?? 0.25
    const filterFactorMax = options.filterFactorMin ?? 0.4
    const filterFactorDelta = options.filterFactorMin ?? 0.05
    let filteredDelays
    let stdDeviation
