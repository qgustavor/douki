...
export default async function (filePath, syncFolder, targetFolder) {
  ...
  for (const syncData of syncedSubtitles) {
    ...
    debugData[path.basename(syncData.subtitlePath)] = JSON.parse(JSON.stringify(matchData))
  ...
  const debugName = subtitleId + '.synced.json'
  const debugPath = path.resolve(targetFolder, debugName)
  await fs.promises.writeFile(debugPath, JSON.stringify(debugData))
