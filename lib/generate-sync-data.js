...
export default async function (data) {
  ...
  const parsedStart = parseTimestamp(start)
  ...
  const audioInfoProcess = cp.spawn('ffprobe', [
    '-show_streams',
    '-print_format', 'json',
    sourceFile
  ], {
  ...
  let audioInfo = ''
  audioInfoProcess.stdout.on('data', e => { audioInfo += e })
  await new Promise((resolve, reject) => {
    audioInfoProcess.on('exit', code => {
  ...
  const includesVideo = audioInfo.streams.some(e => e.codec_type === 'video')
  let keyframes = []
  ...
  const ffmpegArgv = needsReencode
    ...
    : [
        '-ss', fixedStart.toFixed(5), '-to', end, '-i', sourceFile,
        '-map', '0',
        '-avoid_negative_ts', 'make_zero',
        '-codec', 'copy',
        croppedPath,
  ...
  if (subtitleFile) {
    await runFfmpeg([
      '-i', subtitleFile, '-ss', fixedStart, '-to', end,
  ...
  if (gotValidSubtitle) {
    const subtitleDuration = parseTimestamp(end) - parsedStart
    subtitleData = subtitleData.replace(/^Dialogue:.*[\r\n]+/gm, e => {
      const parts = e.split(',')
      if (parseTimestamp(parts[1]) < subtitleDuration) return e
      return ''
    })
    gotValidSubtitle = (subtitleData.match(/^Dialogue: /gm) || []).length > 10
  }

  if (gotValidSubtitle) {
    subtitleData = subtitleData
      .replace(/\[Aegisub Project Garbage]\[^\[]+?/, '')
      .replace(/\[V4\+ Styles\]/, `[Aegisub Project Garbage]
Audio File: ${subtitleId}.mkv${
  includesVideo
? `
Video File: ${subtitleId}.mkv
Keyframes File: ${subtitleId}-keyframes.txt`
: ''
}
...
[V4+ Styles]`)
    ...
    const subtitleData = templateData
      .replace(/---TITLE---/, subtitleId)
      .replace(/\[V4\+ Styles\]/, `[Aegisub Project Garbage]
Audio File: ${subtitleId}.mkv${
  includesVideo
? `
